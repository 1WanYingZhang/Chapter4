...
from CDF_MADRL import (
    CDFMADRLAgent,
    CDFObservationBuilder,
    CDFObservationConfig,
    CDFRewardConfig,
    CDFRewardModulator,
    MAPPOConfig,
    RolloutBuffer,
...
#
# 1) Map file:
#    By default, train_CDF_MADRL.py uses MAPPO_Multiagent_Evacuation/map.txt.
#
# 2) Exit locations:
...
MAP_FILE = Path(__file__).resolve().parent / "空白地图.txt"

# Fill all internal exits here. Every exit must be a 0 cell in map.txt.
# Example:
#EXITS = [(30, 120), (45, 210), (70, 350)]
# 如果终点为空，也就是不手动传 --exits，并且 train_CDF_MADRL.py 里的 EXITS = []，程序会自动读取地图 txt 中数值为 2 的格子作为终点
EXITS: list[tuple[int, int]] = []
# 如果起点为空，也就是不手动传 --starts，并且 train_CDF_MADRL.py 里的 STARTS = []，程序会根据 NUM_AGENTS / --num-agents 在可通行区域随机选择不重复的智能体起点
# Number of evacuees/agents.
NUM_AGENTS = 30

# Fill exact starts here if you want fixed initial positions.
# If STARTS is empty, train_CDF_MADRL.py automatically chooses NUM_AGENTS free cells far from exits.
# If STARTS is not empty, its length must equal NUM_AGENTS.
# Example:
#STARTS = [(90, 50), (91, 50), (92, 50), (93, 50), (94, 50), (95, 50), (96, 50), (97, 50)]

STARTS: list[tuple[int, int]] = [(41, 31), (12, 21), (2, 8), (33, 29), (35, 34), (31, 0), (4, 6), (42, 47), (32, 29), (32, 2), (39, 28), (35, 4), (10, 41), (21, 11), (16, 22), (26, 11), (28, 29), (35, 40), (6, 7), (25, 27), (32, 43), (4, 29), (31, 36), (33, 24), (19, 23), (19, 44), (6, 5), (28, 33), (46, 37), (15, 41)]
...
# CDF-MADRL settings.
# alpha: number of neighboring agents observed by each agent.
# beta: Manhattan local observation range.
# chi: dynamic threshold for switching distance-only vs distance+exit-congestion reward.
# R_arr is not numerically fixed in the paper; use the MAPPO goal reward scale for fair comparison.
CDF_OBSERVED_AGENTS_ALPHA = 35
CDF_OBSERVATION_RANGE_BETA = 5
CDF_DYNAMIC_THRESHOLD_CHI = 30
CDF_ARRIVAL_REWARD = 30.0
...
def build_map_txt_env(args) -> MultiAgentGridEvacuationEnv:
    ...
    if not exits:
        raise ValueError(
            "No exits were configured. Set EXITS in train_CDF_MADRL.py or mark exits as 2 in the map txt."
        )
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train CDF-MADRL on MAPPO_Multiagent_Evacuation/map.txt with explicit internal exits."
    )
    ...
    parser.add_argument("--cdf-observed-agents-alpha", type=int, default=CDF_OBSERVED_AGENTS_ALPHA)
    parser.add_argument("--cdf-observation-range-beta", type=int, default=CDF_OBSERVATION_RANGE_BETA)
    parser.add_argument("--cdf-dynamic-threshold-chi", type=int, default=CDF_DYNAMIC_THRESHOLD_CHI)
    parser.add_argument("--cdf-arrival-reward", type=float, default=CDF_ARRIVAL_REWARD)
    return parser
...
def make_code_config_args() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        clip_coef=CLIP_COEF,
        cdf_observed_agents_alpha=CDF_OBSERVED_AGENTS_ALPHA,
        cdf_observation_range_beta=CDF_OBSERVATION_RANGE_BETA,
        cdf_dynamic_threshold_chi=CDF_DYNAMIC_THRESHOLD_CHI,
        cdf_arrival_reward=CDF_ARRIVAL_REWARD,
    )
...
def main(seed_override: int | None = None) -> None:
    ...
    save_dir = build_algorithm_save_dir(args, "CDF_MADRL", run_id)
    ...
    obs_builder = CDFObservationBuilder(
        env.grid_map,
        env.exits,
        CDFObservationConfig(
            observed_agents=args.cdf_observed_agents_alpha,
            observation_range=args.cdf_observation_range_beta,
        ),
    )
    agent = CDFMADRLAgent(obs_builder.obs_dim, env.n_actions, env.n_agents, cfg)
    env_cfg = getattr(env, "cfg", EvacuationConfig())
    reward_scale = abs(float(env_cfg.step_penalty))
    reward_modulator = CDFRewardModulator(
        CDFRewardConfig(
            dynamic_threshold=args.cdf_dynamic_threshold_chi,
            arrival_reward=args.cdf_arrival_reward,
            distance_reward_scale=reward_scale,
            global_reward_scale=reward_scale,
            max_steps=args.max_steps,
            max_distance=float(max(1, env.rows + env.cols - 2)),
        )
    )

    print(f"Use code config: {USE_CODE_CONFIG}")
    print("Algorithm: CDF-MADRL")
    ...
    print(f"CDF obs dim: {obs_builder.obs_dim}")
    print(f"CDF action dim: {env.n_actions}")
    ...
    experiment_config = {
        ...
        "seed": args.seed,
        "cdf_observation_config": {
            "observed_agents_alpha": args.cdf_observed_agents_alpha,
            "observation_range_beta": args.cdf_observation_range_beta,
            "obs_dim": obs_builder.obs_dim,
            "entity_triples": obs_builder.num_triples,
        },
        "cdf_reward_config": reward_modulator.to_dict(),
    }
    ...
    _, infos = vec_env.reset(seed=args.seed)
    obs = obs_builder.transform(infos)
    reward_modulator.reset(infos, env.exits)
    ...
    for update in range(1, num_updates + 1):
        ...
        for _ in range(cfg.rollout_steps):
            ...
            flat_obs = obs_tensor.reshape(vec_env.num_envs * env.n_agents, agent.obs_dim)
            ...
            prev_infos = infos
            _, base_rewards, terminated, truncated, next_infos = vec_env.step(actions.cpu().numpy())
            next_obs = obs_builder.transform(next_infos)
            rewards = reward_modulator.shape(base_rewards, prev_infos, next_infos, env.exits)
            ...
            for env_idx in np.where(done)[0]:
                ...
                _, reset_info = vec_env.reset_at(int(env_idx))
                reward_modulator.reset_at(int(env_idx))
                obs[env_idx] = obs_builder.transform_one(reset_info)
