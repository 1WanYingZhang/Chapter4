...
from RMAPPO import RMAPPOAgent, RMAPPOConfig, RecurrentRolloutBuffer
...
#
# 1) Map file:
#    By default, train_RMAPPO.py uses MAPPO_Multiagent_Evacuation/map.txt.
#
# 2) Exit locations:
...
# 如果终点为空，也就是不手动传 --exits，并且 train_RMAPPO.py 里的 EXITS = []，程序会自动读取地图 txt 中数值为 2 的格子作为终点
EXITS: list[tuple[int, int]] = []
# 如果起点为空，也就是不手动传 --starts，并且 train_RMAPPO.py 里的 STARTS = []，程序会根据 NUM_AGENTS / --num-agents 在可通行区域随机选择不重复的智能体起点
# Number of evacuees/agents.
NUM_AGENTS = 30

# Fill exact starts here if you want fixed initial positions.
# If STARTS is empty, train_RMAPPO.py automatically chooses NUM_AGENTS free cells far from exits.
# If STARTS is not empty, its length must equal NUM_AGENTS.
# Example:
#STARTS = [(90, 50), (91, 50), (92, 50), (93, 50), (94, 50), (95, 50), (96, 50), (97, 50)]

STARTS: list[tuple[int, int]] = [(30, 23), (6, 14), (26, 26), (8, 10), (30, 9), (21, 43), (4, 21), (42, 22), (2, 9), (35, 33), (6, 17), (42, 20), (27, 4), (14, 17), (14, 10), (30, 26), (38, 22), (15, 20), (28, 29), (27, 30), (30, 33), (46, 33), (8, 24), (21, 8), (2, 19), (23, 44), (26, 42), (19, 7), (13, 40), (46, 26)]
...
def build_map_txt_env(args) -> MultiAgentGridEvacuationEnv:
    ...
    if not exits:
        raise ValueError(
            "No exits were configured. Set EXITS in train_RMAPPO.py or mark exits as 2 in the map txt."
        )
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train RMAPPO on MAPPO_Multiagent_Evacuation/map.txt with explicit internal exits."
    )
    ...
    parser.add_argument("--recurrent-layers", type=int, default=RECURRENT_LAYERS)
    return parser
...
def main(seed_override: int | None = None) -> None:
    ...
    save_dir = build_algorithm_save_dir(args, "RMAPPO", run_id)
    ...
    agent = RMAPPOAgent(env.obs_dim, env.n_actions, env.n_agents, cfg)

    print(f"Use code config: {USE_CODE_CONFIG}")
    print("Algorithm: RMAPPO")
    ...
    actor_rnn_states, critic_rnn_states = agent.initial_states(vec_env.num_envs)
    ...
    for update in range(1, num_updates + 1):
        ...
        for _ in range(cfg.rollout_steps):
            ...
            actor_states_before = actor_rnn_states.clone()
            critic_states_before = critic_rnn_states.clone()
            with torch.no_grad():
                actions, logprobs, values, next_actor_rnn_states, next_critic_rnn_states = agent.act(
                    obs_tensor,
                    global_obs,
                    actor_rnn_states,
                    critic_rnn_states,
            ...
            obs = next_obs
            infos = next_infos
            actor_rnn_states = next_actor_rnn_states
            critic_rnn_states = next_critic_rnn_states
            ...
            for env_idx in np.where(done)[0]:
                ...
                actor_rnn_states[env_idx].zero_()
                critic_rnn_states[env_idx].zero_()
        ...
        with torch.no_grad():
            next_value, _ = agent.critic(global_obs, critic_rnn_states, current_active_mask)
