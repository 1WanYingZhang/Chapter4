...
class EvacuationConfig:
    ...
    non_target_exit_reward_ratio: float = 1.0
    team_completion_reward: float = 50.0
...
class MultiAgentGridEvacuationEnv(gym.Env):
    ...
    def __init__(
        ...
        self.exit_distance_map, self.nearest_exit_map = self._build_exit_distance_map()
        reachable_exit_distances = self.exit_distance_map[self.exit_distance_map >= 0]
        self.max_exit_distance = (
            float(np.max(reachable_exit_distances)) if reachable_exit_distances.size else self._diagonal
        )
    ...
    def step(self, actions: Sequence[int]):
        ...
        escaped_this_step = 0
        escaped_agent_ids: list[int] = []
        escaped_positions: list[list[int]] = []
        for i in range(self.n_agents):
            if not self.active[i]:
            ...
            if self._is_exit(self.agent_pos[i]):
                ...
                components[i]["exit"] += exit_reward
                rewards[i] += exit_reward
                escaped_this_step += 1
                escaped_agent_ids.append(int(i))
                escaped_positions.append([int(self.agent_pos[i, 0]), int(self.agent_pos[i, 1])])
        ...
        info = self._info(
            ...
            include_paths=terminated,
            escaped_agent_ids=escaped_agent_ids,
    ...
    def _build_exit_distance_map(self) -> tuple[np.ndarray, np.ndarray]:
        distances = np.full((self.rows, self.cols), -1, dtype=np.int32)
        nearest = np.full((self.rows, self.cols, 2), -1, dtype=np.int32)
        queue: deque[tuple[int, int]] = deque()

        for exit_point in self.exits:
            r, c = int(exit_point[0]), int(exit_point[1])
            if distances[r, c] >= 0:
                continue
            distances[r, c] = 0
            nearest[r, c] = np.array([r, c], dtype=np.int32)
            queue.append((r, c))

        while queue:
            r, c = queue.popleft()
            current = np.array([r, c], dtype=np.int32)
            for move in self.BASE_MOVES:
                previous = current + move
                if self._out_of_bounds(previous) or self._blocked(previous):
                    continue
                reverse_move = current - previous
                if self._move_blocked(previous, reverse_move):
                    continue
                pr, pc = int(previous[0]), int(previous[1])
                if distances[pr, pc] >= 0:
                    continue
                distances[pr, pc] = distances[r, c] + 1
                nearest[pr, pc] = nearest[r, c]
                queue.append((pr, pc))

        return distances, nearest

    def _euclidean_nearest_exit(self, point: np.ndarray) -> np.ndarray:
        deltas = self.exits - point
        idx = int(np.argmin(np.linalg.norm(deltas, axis=1)))
        return self.exits[idx].copy()

    def _nearest_exit(self, point: np.ndarray) -> np.ndarray:
        if not self._out_of_bounds(point):
            r, c = int(point[0]), int(point[1])
            exit_point = self.nearest_exit_map[r, c]
            if int(exit_point[0]) >= 0:
                return exit_point.copy()
        return self._euclidean_nearest_exit(point)

    def _distance_to_nearest_exit(self, point: np.ndarray) -> float:
        if not self._out_of_bounds(point):
            distance = int(self.exit_distance_map[int(point[0]), int(point[1])])
            if distance >= 0:
    ...
    def _distance_to_assigned_exit(self, agent_id: int, point: np.ndarray) -> float:
        return self._distance_to_nearest_exit(point)

    def _update_assigned_exits(self) -> None:
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        for i in range(self.n_agents):
            ...
            dy = float(delta[0])
            dx = float(delta[1])
            coord_dist = float(np.linalg.norm(delta))
            path_dist = self._distance_to_nearest_exit(self.agent_pos[i])
            if coord_dist > 1e-6:
                dx_unit = dx / coord_dist
                dy_unit = dy / coord_dist
            else:
                dx_unit = 0.0
                dy_unit = 0.0
            goal_vector = np.array(
                [
                    ...
                    np.clip(path_dist / max(1.0, self.max_exit_distance), 0.0, 1.0),
                ],
                dtype=np.float32,
            )
            ...
            local_exit_visible = False
            for exit_point in self.exits:
                exit_delta = exit_point - self.agent_pos[i]
                exit_dr = int(exit_delta[0])
                exit_dc = int(exit_delta[1])
                if abs(exit_dr) <= radius and abs(exit_dc) <= radius:
                    local_maps[2, radius + exit_dr, radius + exit_dc] = 1.0
                    local_exit_visible = True

            if not local_exit_visible:
    ...
    def _info(
        self,
        escaped_this_step: int,
        include_paths: bool = False,
        escaped_agent_ids: Sequence[int] | None = None,
        escaped_positions: Sequence[Sequence[int]] | None = None,
    ) -> Dict:
        escaped_agent_ids = list(escaped_agent_ids or [])
        escaped_positions = list(escaped_positions or [])
        info = {
            ...
            "escaped_agent_ids": np.asarray(escaped_agent_ids, dtype=np.int32),
            "escaped_positions": np.asarray(escaped_positions, dtype=np.int32).reshape(-1, 2),
