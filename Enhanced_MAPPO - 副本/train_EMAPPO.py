...
from emappo import MAPPOAgent, MAPPOConfig, RolloutBuffer
from emappo_env import EvacuationConfig, MultiAgentGridEvacuationEnv
from train import _points_to_arg, build_algorithm_save_dir
...
EXITS: list[tuple[int, int]] = [(0, 25), (40, 49)]
...
def _append_episode_metrics(lines: list[str], title: str, metrics: dict | None) -> None:
    ...
    lines.append("  arrival_steps:")
    for idx, step in enumerate(metrics["arrival_steps"]):
        lines.append(f"    agent_{idx}: {step}")
    lines.append("  path_lengths:")
    for idx, length in enumerate(metrics["path_lengths"]):
        lines.append(f"    agent_{idx}: {length:.6f}")
    lines.append(f"  average_path_length: {metrics['average_path_length']:.6f}")
    lines.append("  collision_counts:")
    for idx, count in enumerate(metrics["collision_counts"]):
        lines.append(f"    agent_{idx}: {count}")
...
def save_agent_metrics_csv(
    ...
    for episode_name, metrics in (("last", last_metrics), ("best", best_metrics)):
        ...
        n_agents = max(len(arrivals), len(path_lengths), len(collisions), len(counts), len(durations))
...
def build_map_txt_env(args) -> MultiAgentGridEvacuationEnv:
    ...
    if not exits:
        raise ValueError(
            "No exits were configured. Set EXITS in train1.py or mark exits as 2 in the map txt."
        )
...
def main() -> None:
    ...
    save_dir = build_algorithm_save_dir(args, "EMAPPO", run_id)
