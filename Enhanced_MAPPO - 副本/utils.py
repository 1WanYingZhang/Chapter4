...
def find_marked_exits(grid: np.ndarray, marker: int = 2) -> List[GridPoint]:
    points = np.argwhere(grid == marker)
    return [(int(row), int(col)) for row, col in points]


def normalize_map_markers(grid: np.ndarray) -> np.ndarray:
    normalized = np.asarray(grid, dtype=np.int8).copy()
...
def choose_random_starts(grid: np.ndarray, exits: Sequence[GridPoint], n_agents: int) -> List[GridPoint]:
    free = np.argwhere(grid == 0)
    exit_set = {tuple(int(x) for x in exit_point) for exit_point in exits}
    candidates = [
        (int(point[0]), int(point[1]))
        for point in free
        if tuple(int(x) for x in point) not in exit_set
    ]
    if len(candidates) < n_agents:
        raise ValueError("Not enough free cells to randomly place all agents.")
    indices = np.random.choice(len(candidates), size=n_agents, replace=False)
