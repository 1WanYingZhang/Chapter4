...
def infer_local_obs_layout(obs_dim: int, map_channels: int = 3, vector_dim: int = 3) -> Tuple[int, int, int]:
    ...
    if image_values <= 0 or image_values % map_channels != 0:
        raise ValueError(f"obs_dim={obs_dim} cannot be split into {map_channels} map channels and {vector_dim} vector values.")
...
class ActorFeatureEncoder(nn.Module):
    """Stronger actor encoder for local spatial policy decisions."""
...
class CriticFeatureEncoder(nn.Module):
    """Lighter per-agent encoder for the centralized critic."""
