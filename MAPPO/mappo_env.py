...
class MultiAgentGridEvacuationEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.cfg.local_view_radius != 5:
            raise ValueError("MAPPO uses a fixed 11x11 local observation, so local_view_radius must be 5.")
        self.fov_size = 2 * self.cfg.local_view_radius + 1
        self.local_cells = self.fov_size**2
        self.local_grid_dim = self.local_cells - 1
        self.goal_vector_dim = 3
        self.obs_dim = self.local_grid_dim + self.goal_vector_dim
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        for i in range(self.n_agents):
            ...
            local_grid = np.zeros(self.local_grid_dim, dtype=np.float32)
            grid_idx = 0
            for dr in range(-radius, radius + 1):
                for dc in range(-radius, radius + 1):
                    ...
                    if self._blocked(cell):
                        local_grid[grid_idx] = 1.0
                    elif key in occupied and occupied[key] != i:
                        local_grid[grid_idx] = 2.0
                    grid_idx += 1

            obs[i] = np.concatenate([local_grid, goal_vector])
        return obs
